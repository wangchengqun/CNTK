...
class MinibatchSource(cntk_py.MinibatchSource):
    ...
    parent class of all minibatch sources.  A `MinibatchSource` can be indexed
    by the stream name, which will return a :class:`MinibatchData` object that
    can be passed e.g. to the
    ...
    Args:
        ...
        randomization_window (int): size of window that reader will shuffle,
          ignored if `randomize` is `False`
        sample_based_randomization_window (`bool`, defaults to `False`):
          specifies how to interpret `randomization_window`. If `True`, the
          size of the randomization window is interpreted as a certain number
          of samples, otherwise -- as a number of chunks. Similarly to
        ...
        epoch_size (`int`, defaults to :const:`~cntk.io.INFINITELY_REPEAT`):
          ...
          entire data sweep (as indicated by the MinibatchSource) and
          parameters change their values on the sweep-by-sweep basis specified
          by the schedule.
          **Important:**
        ...
        distributed_after (int, defaults to cntk.io.INFINITE_SAMPLES): sample
          count after which minibatch source becomes distributed
    ...
    def next_minibatch(self, minibatch_size_in_samples,
        ...
        Args:
            ...
            input_map (dict): mapping of :class:`~cntk.ops.variables.Variable`
              to :class:`~cntk.cntk_py.StreamInformation` which will be used to
              convert the returned data.
            device (`DeviceDescriptor`, defaults to `None`): CNTK
              DeviceDescriptor
            num_data_partitions: Used for distributed training, indicates into
              how many partitions the source should split the data.
        ...
        Returns:
            ...
            A mapping of :class:`~cntk.cntk_py.StreamInformation` to
            :class:`MinibatchData` if `input_map` was not specified. Otherwise,
            the returned value will be a mapping of
            :class:`~cntk.ops.variables.Variable` to class:`MinibatchData`.
        '''
        if device is None:
    ...
    def restore_from_checkpoint(self, checkpoint):
        ...
        Args:
            checkpoint (:class:`~cntk_py.Dictionary`): checkpoint to restore
              from
        '''
    ...
    def current_position(self):
        ...
        Returns:
            Minibatch position :class:`~cntk.cntk_py.Dictionary` on the global
            timeline.
        '''
...
def _py_dict_to_cntk_dict(py_dict):
    ...
    Returns:
        cntk_py.Dictionary:
        A :class:`~cntk_py.Dictionary` that has been converted from the input
        `dict`
    '''
...
# TODO: This should be a private function; use
# MinibatchSource(deserializer, ...).
@typemap
def _minibatch_source(config):
    '''
    Instantiate the CNTK built-in composite minibatch source which is used to
    stream data into the network.

    Args:
        config (dict): a dictionary containing all the key-value configuration
        entries.
...
class _ReaderConfig(dict):
    ...
    Args:
        ...
        randomize (`bool`, defaults to `True`): randomize images before every
          epoch
        randomization_window (int): size of window that reader will shuffle,
          ignored if `randomize` is `False`
        sample_based_randomization_window (bool, defaults to `False`):
          specifies how to interpret `randomization_range`. If `True`, the size
          of the randomization window is interpreted as a certain number of
          samples, otherwise -- as a number of chunks. Similarly to
          `randomization_window`, this parameter is ignored, when `randomize`
          is `False`
        epoch_size (`int`, defaults to `cntk.io.INFINITELY_REPEAT`): number of
          samples as a scheduling unit.
          Parameters in the schedule change their values every `epoch_size`
          samples. If no `epoch_size` is provided, this parameter is
          substituted by the size of the full data sweep with infinte repeat,
          in which case the scheduling unit is the entire data sweep (as
          indicated by the MinibatchSource) and parameters
        ...
        distributed_after (int, defaults to `cntk.io.INFINITE_SAMPLES`): sample
          count after which reader becomes distributed
        multithreaded_deserializer (`bool`, defaults to `None`): using multi
          threaded deserializer
        frame_mode (`bool`, defaults to `False`): Specifies if data should be
        ...
        truncation_length (`int`): Specifies the truncation length in samples
          for BPTT (positive integer). When using truncation, frame mode cannot
          be used at the same time.
    '''
    ...
    def __init__(self,
                 ...
                 truncated=False,
                 truncation_length=0):
        self['epochSize'] = cntk_py.SizeTWrapper(
            epoch_size)  # force to store in size_t
        if not isinstance(deserializers, (list, tuple)):
            deserializers = [deserializers]
        self['deserializers'] = self.deserializers = deserializers or []
        self['randomize'] = randomize
        self['randomizationWindow'] = cntk_py.SizeTWrapper(
            randomization_window)
        self[
            'sampleBasedRandomizationWindow'] = sample_based_randomization_window
        self['distributedAfterSampleCount'] = cntk_py.SizeTWrapper(
            distributed_after)
        ...
        if truncation_length > 0:
            ...
            if frame_mode:
                raise ValueError(
                    "FrameMode and truncated BPTT are mutually exclusive.")
    ...
    def minibatch_source(self):
        ...
        the graph nodes or the `train_minibatch()` of
...
def HTKMLFDeserializer(label_mapping_file, streams):
    ...
    for stream_name, stream in streams.items():
        if stream.stream_alias is not None:
            raise ValueError("HTKMLFDeserializer does not support steam names")
        dimension = stream.dim
...
# returns a record { stream_alias, is_sparse, optional shape, optional
# transforms, optional context, optional scp, optional mlf }
